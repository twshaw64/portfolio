...
<body>
    ...
    <div class="retrieveForm">
        <form align = "center">
            <?php
                $studentID = "SELECT DISTINCT studentID FROM locationTable";
                $location = $_POST['location'];

                $test = "SELECT * FROM locationTable WHERE studentID LIKE $studentID";
                $check = "SELECT * FROM locationTable WHERE location LIKE ? AND updateTime = (SELECT MAX(updateTime) FROM locationTable WHERE studentID = 565565)";
                ...
                $query = "SELECT * FROM locationTable WHERE ID IN (SELECT MAX(ID) FROM locationTable GROUP BY studentID) AND location LIKE ?";
